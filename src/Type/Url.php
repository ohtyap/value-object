 ...
 * @psalm-immutable
 */
final class Url implements ValueObjectInterface, \Stringable, TransformableInterface
{
    private string $url;
    ...
    public function __construct(string $url)
    {
        if (\filter_var($url, FILTER_VALIDATE_URL) === false) {
            throw new InvalidArgumentException(\sprintf("The given string '%s' is not a valid url.", $url));
        }
        $this->url = $url;
    }
